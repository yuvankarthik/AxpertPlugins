...
const AI_LOGO_SRC = "../HTMLPages/images/axibot.png";
...
function handleAuthFailure(provider, resStatus) {
    ...
    if (resStatus === 401 || resStatus === 403) {
        ...
        throw new Error(
            `Authentication failed for ${provider}. Your API key may be invalid/rotated. Please reconnect in AXI CONNECT.`
        );
    }
}
...
async function callOpenAI(messages, datasetContext) {
    ...
    // USE GLOBAL FUNCTION - Check if it exists, fallback to default
    let basePrompt;
    if (typeof window.getActiveSystemPrompt === 'function') {
        basePrompt = window.getActiveSystemPrompt();
    } else {
        // Fallback in case the function isn't loaded yet
        basePrompt = `You are AXI, an expert Data Analyst and Report Generator.
...
- Include specific numbers, percentages, and data points`; // Your default prompt
        console.warn('getActiveSystemPrompt not found, using default');
    }
...
function createMessageNode(role, content, isMarkdown, ts) {
    ...
    // If role is 'user', we MUST have 'message--user'. 
    // If 'assistant', we use 'message--assistant'.
    ...
    if (role === 'user') {
        ...
        avatarHTML = `
            <div class="message__avatar">
                <img src="../HTMLPages/images/axibot.png">
            </div>`;
    }
    ...
    if (isMarkdown && typeof marked !== 'undefined') {
        ...
        bodyHTML = String(content).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>");
    }

    // 5. Construct the HTML String
    // CRITICAL: The outer div MUST have the roleClass ('message--user') for CSS to work
    return `
...
async function handleSend() {
    ...
    // --- 2. Read File Content (Context Injection) - FULL FILE ---
    let fileContext = "";
    if (attachments.length > 0) {
        ...
        try {
            ...
            // NO LIMIT - Send entire file to AI
            // Note: GPT-4o-mini supports 128k tokens (~400k characters)
            // For very large files (>500kb), consider chunking or using Assistants API
            fileContext = `
\n--- SYSTEM: FILE CONTENT ATTACHED ---
File Name: ${file.name}
Total Size: ${rawText.length} chars
Data Content:
${rawText}
--- END FILE CONTENT ---
`;
            ...
            fileContext = `\n[System: Failed to read file ${file.name}]`;
        }
    }
    ...
    if (!assistantMsg) {
        console.error("Failed to find assistant placeholder");
        setBusy(false);
        return;
    }
    ...
    try {
        ...
        // --- 9. NEW: Detect & Handle Charts (Single OR Multiple) ---
        let finalContent = answer;
        let chartDataList = [];

        // Clean up fences and comments
        ...
        if (jsonCandidate) {
            try {
                ...
                // Case A: Single Chart { "chart": {...} }
                if (parsed.chart) {
                    chartDataList.push(parsed.chart);
                }
                // Case B: Multiple Charts { "charts": [ { "chart": ... }, ... ] }
                else if (parsed.charts && Array.isArray(parsed.charts)) {
                    parsed.charts.forEach(item => {
                        // Some prompts return { chart: {...} } inside the array
                        if (item.chart) chartDataList.push(item.chart);
                        // Some prompts return just the chart object directly
                        else if (item.type && item.series) chartDataList.push(item);
                    });
                }

                if (chartDataList.length > 0) {
                    finalContent = `**Analysis Complete**\n\nI have generated ${chartDataList.length} chart(s) based on the data.`;
                }
            } catch (e) {
                console.warn("Failed to parse chart JSON snippet:", e, jsonCandidate);
            }
        }
        ...
        // Save ALL charts to the message spec so they persist on reload
        if (chartDataList.length > 0) {
            assistantMsg.chartSpecs = chartDataList; // Saving as Plural 'chartSpecs'
        }

        saveChats();
        renderThread();

        // --- 10. Post-Render Actions (Render Charts & Download Buttons) ---
        setTimeout(() => {
            // Find the DOM element for this specific message (it's the last one)
            const allMsgs = document.querySelectorAll(".msg.assistant .bubble");
            const lastBubble = allMsgs[allMsgs.length - 1];

            if (lastBubble) {
                // Render ALL charts if present
                if (chartDataList.length > 0 && typeof renderHighchartInMessage === 'function') {
                    chartDataList.forEach(spec => {
                        renderHighchartInMessage(lastBubble, spec);
                    });
                }

                // Add "Export PDF / Copy" buttons for reports
