<<
CREATE OR REPLACE FUNCTION fn_axi_get_fieldvalues_with_keysuffix_list_axitest
(
    p_tstruct text,
    p_fieldname text
) 
...
<<
CREATE OR REPLACE FUNCTION fn_axi_getkeyvalueswithfieldnameslist_axitest(p_tstruct text, p_fieldname text)
...
<<
CREATE OR REPLACE FUNCTION fn_upsert_config_by_condition_axitest(
    p_tablename      text,
    p_fieldnames     text,
    p_fieldvalues    text,
    p_where_clause   text
...
<<
CREATE OR REPLACE FUNCTION get_ads_dropdown_data_axitest(
    p_tablename  TEXT,
    p_fieldname  TEXT
)
RETURNS TABLE (
...
<<
CREATE OR REPLACE FUNCTION get_dynamic_field_axitest(p_tstruct text, p_fieldname text)
...
<<
CREATE OR REPLACE FUNCTION populate_axdirectsql_metadata_axitest()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
<<
CREATE OR REPLACE FUNCTION fn_axi_metadata_axitest(pstructtype character varying, pusername character varying)
...
<<
CREATE OR REPLACE FUNCTION fn_axi_struct_metadata_axitest(pstructtype character varying, ptransid character varying, pobjtype character varying)
...
<<
CREATE OR REPLACE FUNCTION axi_firesql_v2_axitest(
    p_sql TEXT,
    p_param_string TEXT,
    p_sourcekey TEXT,
    p_fromlist TEXT
